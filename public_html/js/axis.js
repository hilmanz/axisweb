...
var pingTimeout, updateObrolanTimeout, postingToTableTimeout, updateStatsTimeout, updateAdminStatusTimeout, cekUnlikeLogOutTimeout ;
...
function update_table(){
	...
	$.each(left,function(x,v){
		...
		if(v.fb_id!=null){
			...
			var imagesPhoto = "https://graph.facebook.com/"+v.fb_id+"/picture";
		}
		if(v.fb_id!=null){
			itemHtml += '				<span id="special_'+v.fb_id+'"></span>';
			itemHtml += '				<div class="status">';
			itemHtml += '					<div class="user">';
			itemHtml += '						<a class="thumb" href="https://www.facebook.com/'+v.fb_id+'"  target="_blank"><img src="'+imagesPhoto+'" /></a>';
			itemHtml += '						<span class="nickname" id="nickname_'+v.fb_id+'">'+v.name+'</span>';
			itemHtml += '					</div>';
			itemHtml += '					<span class="text wordWarp" id="post_'+v.fb_id+'"><img style="border:0;margin:10px auto 0;" src="img/loader_small.gif"></span>';
...
function update_table_old(){
	...
	$.each(left,function(x,v){
		...
		var img = $("<img>");
		if(v.fb_id!=null){
			img.attr("src","https://graph.facebook.com/"+v.fb_id+"/picture");
		}
		var label = $("<p>");
...
function send_ping(old_pt){
	$.ajax({
		  ...
		  success: function( response ) {
				...
				updateObrolanTimeout = setTimeout(updateObrolan,2*1000);
		  }
	});
}
